...
V2의 window/multi-array/block-phase에 두 보수적 계보를 추가한다. 출처 revision은
...
- 두 역할은 `staged-butterfly` 하나의 계보다. n=2의 A는 각 위치를 한 번만 읽어
...
- 두 역할은 동일한 첫 등장 정규화 재사용 순서를 가지며 **공간 배치 대비**다.
  다른 temporal reuse나 독립 family로 세지 않는다. 둘 모두 `triangular-solve` 계보,
