"""Replay candidate identities, duplicate exclusions and staged ELF references."""
...
def revalidate(output: Path) -> dict:
    """Extract into a fresh directory and reject changed inputs or references."""
    ...
    for key, filename in (('source_manifest_hash', 'recipe-sources.json'),
                          ('staged_source_manifest_hash', 'staged-recipe-sources.json')):
        assert pool['design'][key] == file_hash(pool_root / 'implementation' / filename)
    ...
    signatures = [input_signature(r['configuration']) for r in candidates]
    assert len(set(signatures)) == len(signatures)
    by_id = {r['workload_id']: r for r in candidates}
    duplicates = pool['duplicate_candidates']
    assert len(candidates)+len(duplicates) == pool['design']['generated_candidate_workloads'] == 300
    for row in duplicates:
        assert row['exclusion_reason'] == 'duplicate_generator_input'
        assert input_signature(row['configuration']) == input_signature(
            by_id[row['duplicate_of']]['configuration'])
    plans = 0
    ...
    result = dict(status='PASS', candidate_workloads=len(candidates), plans=plans,
        primary_families=registry['primary_family_count'], duplicate_candidates=len(duplicates),
        linked_streams=streams, reference_accesses_per_architecture=sum(map(len, references.values())),
        fixture_tasks=len(references), measured_workloads=0, dataset_ready=False, split_frozen=False)
    assert (len(candidates), len(duplicates), plans, streams) == (207, 93, 621, 24)
