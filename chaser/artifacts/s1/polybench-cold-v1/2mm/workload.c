...
volatile double A[NI][NK];
volatile double B[NK][NJ];
volatile double C[NJ][NL];
volatile double D[NI][NL];
volatile double tmp[NI][NJ];  // 중간 결과

__attribute__((noinline,annotate("ape.analyze"))) void kernel_2mm(double alpha, double beta) {
    int i, j, k;
