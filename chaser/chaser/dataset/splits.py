...
def taskset_signature(config: dict) -> str:
    """Identify ordered task work across names, policy and core placement changes."""
    ...
    payload['tasks'] = [{key: value for key, value in task.items()
                         if key not in ('task_id', 'core')} for task in config['tasks']]
